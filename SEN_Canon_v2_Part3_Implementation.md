...
## 第11章：LLM いいとこ取り OS（能力バンクとモデル非依存設計）

第4章が「今あるAIをどう三層に配置するか」を扱うのに対し、
本章は「特定のモデルに依存しない構造をどう作るか」を扱う。

問いの起点はこうである。

> LLM を使う／使わないに関わらず、
> あるいはローカル LLM でも、性能はだんだん良くなってくる。
> ならば、特定のモデルに依存しない仕組み化をした方がいいのではないか。

SEN Design の答えは明確である。**その方向が本命である。**

### 11.1 なぜモデル非依存が本命か

第3条「空の実装（何も固定されない）」と、
第7条「抽象と具体の非対称性」から導かれる。

- モデルは1〜2年で陳腐化する。**点（一時の最良モデル）に賭けてはいけない。**
- 残る資産は、溜めた知識・手順・評価基準＝**線（流れ）**である。
- 器（ワークフロー、能力バンク、評価セット）を抽象側に保てば、
  中身の LLM を差し替えるだけで性能が上がっていく。

モデルは借り物である（第9条の応用）。
良く使い、良く返す。執着しない。

### 11.2 能力バンク（Capability Bank）

LLM ごとの得意技を、特定モデルの外側に抽象化して溜める器。

| 蓄積する対象 | 内容 | どこに残るか |
|------------|------|------------|
| 手順（プレイブック） | 調査・生成・要約等のワークフロー | Vault（Obsidian 等）|
| 文脈（コンテキスト） | プロジェクト固有の前提・制約 | 境界線の内側（第5条）|
| 評価基準（ルーブリック） | 何を「良い」とするかの定義 | 能力バンク |
| 調査キャプチャ | 過去の調査結果・参照の蓄積 | 能力バンク |

ポイントは、**これらがモデルの外にあること**。
モデルを乗り換えても、バンクはそのまま次のモデルに接続できる。

### 11.3 二層構造：共通インターフェース＋モデル別アダプタ

「完全な抽象化」は幻想になりやすい。
LLM ごとに、得意技もプロンプトの最適点も違う。
均しすぎると、各モデルの良さも潰れる。

ゆえに二層で設計する。

```
【共通層】タスクの抽象定義（モデルを知らない）
   - 「何を達成したいか」だけを記述
   - 能力バンク・評価基準・文脈はここに接続
        ↓
【アダプタ層】モデル別の最適化（モデルを知っている）
   - Claude 向け / Codex 向け / ローカル LLM 向け
   - プロンプト整形・ツール呼び出し・出力規約を吸収
```

共通層は安定資産（線）。
アダプタ層は差し替え可能な消耗品（点）。
この非対称を守ることが、移行コストを下げる。

### 11.4 オーケストレーション：役割分担とルーティング
...
| タスクの性質 | 振り先の例 | 根拠 |
|------------|----------|------|
| 長文脈・設計・コード | クラウド最上位モデル | 得意技の活用 |
| 機密データの処理 | ローカル LLM 必須 | 第5条 データ主権 |
| 定型・反復 | 軽量／ローカル | コストと主権 |
| 別視点の補完 | 別ベンダー系 | 第15条 視差の補完 |

**どのタスクをどのモデルに振るか**の判断ロジックそのものが、
乗り換え可能なモデルよりも重要な資産になる。

### 11.5 ルーティングを評価データで磨く

ルーティングを勘で固定しない。
「どの構成が、どのタスクで良かったか」を記録し、判断を改善し続ける。

1. **評価セットを持つ**：代表的なタスク群と、良し悪しの基準を定義
2. **構成を記録する**：どのモデル／アダプタで、どの結果が出たか
3. **差分で判断する**：新モデルが出たら、評価セットに通して比較
4. **ルーティングを更新する**：勝った構成へ振り分けを寄せる

これにより、モデルが進化するほど、
バンクとルーティングが自動的に良くなる構造になる。
人間は具体（個別の最適化）に降りず、抽象（評価基準の設計）に留まる（第5章）。

### 11.6 この設計が守る原則

- **第3条**：最良モデルを固定しない。空のまま、流れに乗せる
- **第5条**：機密はローカル、難所だけクラウド。境界線を越えさせない
- **第7条**：抽象（共通層・評価基準）が具体（モデル）を引き上げる
- **第6条**：時間・蓄積・評価基準は、コピーできない壁になる

最良の LLM を追いかけるのではなく、
**どの LLM が来ても、いいとこ取りできる器**を作る。
それが「LLM いいとこ取り OS」である。
